...
```{r}
t.test(Rendimiento~Especie , data=df_coriandrum, alternative = "greater", var.equal = T, conf.level = 0.95)
...
```{r}
qqnorm(df_avena$diferencia)
qqline(df_avena$diferencia, col = 2)
```

```{r}
data <-read.delim2("Avena.txt")
data$Tratamiento <-as.factor(data$Tratamiento)
data$Altura <-as.numeric(data$Altura)
t.test(data$Altura~data$Tratamiento, alternative = "greater", paired = T)
```
...
mood.medtest(Aceite~Variedad, data=aceite)# Realiza el test de la mediana de Mood
```
...
```{r}
modelo<-aov(cd_tallo ~ factor(dosis_cd), data=cadmio)
summary(modelo)
```
...
```{r}
kruskalmc(Puntajes~Grupo )# Realiza un test de comparación múltiple entre tratamientos luego del test de Kruskal Wallis
```
