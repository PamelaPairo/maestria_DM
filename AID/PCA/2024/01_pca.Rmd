...
df_sample$education_Graduation <- as.factor(df_sample$education_Graduation)
```
...
g <-ggpairs(df_num, 
            aes(color = df_sample$marital_Married, alpha = 0.5),
...
```{r}
ggplot(data_long, aes(x=variable, y=value, fill= marital_Single)) + 
    geom_boxplot() +
    facet_wrap(~variable, scale="free")
```
...
```{r}
scatter3D(df_sample$NumStorePurchases, df_sample$MntSweetProducts, df_sample$Recency, 
          phi = 0, 
          bty = "g",
          pch = 20, 
          cex = 2, 
          ...
          col = "blue",
          xlab="MntFruits",
          ylab="MntSweetProducts",
          zlab="Recency")
```

```{r}
mycolors <- c('royalblue1', 'red')
df_sample$color <- mycolors[ as.numeric(df_sample$marital_Single) ]

plot3d(df_sample$NumStorePurchases, df_sample$MntSweetProducts, df_sample$Recency, 
          type = 's', 
          radius = 1.7,
          col = df_sample$color,
          xlab="MntFruits",
