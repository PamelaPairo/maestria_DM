...
$$
  {\LARGE s^{2}={\frac{{\displaystyle \sum_{i=1}^{N}\left(x_{i}- \overline{x}\right)^{2}}}{n-1}}}
$$
...
```{r}
m_cov <- eigen(m_cov)
autovalores_cov <- m_cov$values #autovalores
...
apply(X = df_numericas, MARGIN = 2, FUN = var)
```
...
autoplot(pca, data = df, colour = 'Poblacion',
