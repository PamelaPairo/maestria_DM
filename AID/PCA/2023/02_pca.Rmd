...
```{r}
library(ggfortify)
autoplot(pca, data = df, shape = F, label.size = 3, scale = 0)# scale=0 biplot segun scores
```
...
df_out<-rbind(as_tibble(Carseats), 
              c (1, 900, 3, 100, 20, 4000, "Good", 5, 10, "No", "No") ,
              c (5, 250, 7, 0, 20, 150, "Good", 20, 10, "Yes", "No") ,
              c ( 20, 15, 20, 500, 20, 1450, "Medium", 10, 50, "No", "No"))
...
```{r}
pca_mcd <-princomp(df_out, cor=TRUE, scores=TRUE,
...
```{r}
pca_mve <-princomp(df_out, cor=TRUE, scores=TRUE,
...
```{r}
screeplot(pca_mcd, type = "l", npcs = 12)
abline(h = 1, col="red", lty=5)
legend("topright", legend=c("Eigenvalue = 1"),
col=c("red"), lty=5, cex=0.6)
