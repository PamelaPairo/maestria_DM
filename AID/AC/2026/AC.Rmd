...
Perfil de la fila i-ésima es la proporción de cada categoría de la variable j en
la fila i.
...
CrossTable(as.matrix(housetasks),
           prop.t=FALSE,
           prop.r=FALSE, #proporciones columnas
           prop.c=TRUE, 
           prop.chisq=FALSE)
...
esperadas, y por lo tanto mayor dependencia entre filas y columnas, mayor inercia, mayor $\chi^{2}$

- Equivale a la **varianza total** en ACP

El método se basa en descomponer la inercia total en ejes en los cuales
se representan las desviaciones en relación a lo esperado bajo independencia. Cada eje explica una fracción de inercia. La inercia equivale al autovalor.

## Biplot

Se busca representar las filas y las columnas de una tabla de contingencia como puntos en un espacio vectorial de baja dimensión (generalmente bidimensional), de forma que los correspondientes espacios se puedan superponer para obtener una representación conjunta.

- Cuando las coordenadas de las categorías de ambas variables son grandes y del mismo signo se deduce una asociación positiva entre las columnas y las filas correspondientes (la combinación de categorías es más frecuente que la esperada por azar bajo independencia)

- Cuando son grandes en valor absoluto, pero de signos opuestos, las filas y columnas correspondientes tienen asociación negativa (la combinación de categorías es menos frecuente que la esperada por azar bajo independencia)

- Cuando están próximas a 0, la asociación entre las categorías es baja o nula
...
¿Cómo evalúo la calidad de la representación?

$cos^2$: Qué tan bien representado está ese punto en ese eje (calidad de representación).

Un valor cercano a 1 indica que el punto está perfectamente representado en ese eje (está "dentro" del plano de la diapositiva).

Un valor cercano a 0 indica que el punto no está bien representado por ese eje (está proyectado desde "muy lejos" en otra dimensión que no estamos viendo).
...
El objetivo es reducir la dimensión del problema y lograr una representación que, perdiendo la menor cantidad de información posible, represente nuestra tabla de contingencias.
